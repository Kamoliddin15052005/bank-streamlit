...
DB_PATH = os.getenv("DB_PATH", "bank_assets.db")
...
def update_asset(asset_id, name, asset_type, category, description):
    conn = get_conn()
    conn.execute(
        "UPDATE assets SET name=?,asset_type=?,category=?,description=?,updated_at=datetime('now') WHERE id=?",
        (name, asset_type, category, description, asset_id)
    )
    conn.execute(
        "INSERT INTO asset_history (asset_id,action,changed_by,details) VALUES (?,?,?,?)",
...
def change_status(asset_id, new_status, reason="", changed_by="admin"):
    ...
    if new_status == "REGISTERED" and old_status == "ASSIGNED":
        conn.execute(
            "UPDATE assignments SET is_active=0,returned_at=datetime('now') WHERE asset_id=? AND is_active=1",
            (asset_id,)
        )
    conn.execute(
...
def create_employee(full_name, employee_id, department, branch="", email="", phone=""):
    ...
    try:
        conn.execute(
            "INSERT INTO employees (full_name,employee_id,department,branch,email,phone) VALUES (?,?,?,?,?,?)",
...
def get_assignments(active_only=True):
    conn = get_conn()
    q = """
        SELECT a.*, ast.name as asset_name, ast.serial_number,
...
def assign_asset(asset_id, employee_id=None, department="", branch="", notes=""):
    ...
    if asset["status"] in ("LOST", "WRITTEN_OFF", "IN_REPAIR"):
        conn.close()
        return False, f"{STATUS_UZ[asset['status']]} aktiv tayinlanmaydi!"
    # Deactivate previous
    conn.execute(
        "UPDATE assignments SET is_active=0,returned_at=datetime('now') WHERE asset_id=? AND is_active=1",
        (asset_id,)
    ...
    conn.execute(
        "INSERT INTO assignments (asset_id,employee_id,department,branch,notes) VALUES (?,?,?,?,?)",
    ...
    conn.execute(
        "INSERT INTO asset_history (asset_id,action,old_status,new_status,changed_by,details) VALUES (?,?,?,?,?,?)",
...
def return_asset(assignment_id, reason="", changed_by="admin"):
    conn = get_conn()
    asgn = conn.execute("SELECT * FROM assignments WHERE id=?", (assignment_id,)).fetchone()
    if not asgn:
        conn.close()
        return False, "Tayinlash topilmadi"
    conn.execute(
        "UPDATE assignments SET is_active=0,returned_at=datetime('now') WHERE id=?",
        (assignment_id,)
    )
    conn.execute(
        "UPDATE assets SET status='REGISTERED',updated_at=datetime('now') WHERE id=?",
        (asgn["asset_id"],)
    )
    conn.execute(
        "INSERT INTO asset_history (asset_id,action,old_status,new_status,changed_by,reason) VALUES (?,?,?,?,?,?)",
...
def get_stats():
    ...
    by_dept = dict(conn.execute(
        "SELECT department, COUNT(*) FROM assignments WHERE is_active=1 AND department!='' GROUP BY department"
    ).fetchall())
    recent = conn.execute(
        "SELECT COUNT(*) FROM asset_history WHERE created_at >= datetime('now','-7 days')"
    ).fetchone()[0]
    conn.close()
...
def get_all_history(limit=50):
    conn = get_conn()
    rows = conn.execute(
        "SELECT h.*, a.name as asset_name FROM asset_history h JOIN assets a ON h.asset_id=a.id ORDER BY h.created_at DESC LIMIT ?",
        (limit,)
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
