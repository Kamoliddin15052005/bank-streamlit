...
<style>
    ...
    [data-testid="stSidebar"] .stRadio label { color: rgba(255,255,255,0.85) !important; font-size: 15px; }
    ...
    .badge-REGISTERED { background:#ebf8ff; color:#2b6cb0; }
    .badge-ASSIGNED    { background:#f0fff4; color:#276749; }
    .badge-IN_REPAIR   { background:#fffbeb; color:#92400e; }
    .badge-LOST        { background:#fff5f5; color:#c53030; }
    .badge-WRITTEN_OFF { background:#f7fafc; color:#718096; }
    ...
    /* Success/error */
    .stSuccess, .stError, .stInfo, .stWarning { border-radius: 8px; }
...
if page == "📊 Dashboard":
    ...
    with col1:
        ...
        if stats['by_status']:
            ...
            color_map = {
                STATUS_UZ["REGISTERED"]: "#3182ce",
                STATUS_UZ["ASSIGNED"]: "#38a169",
                STATUS_UZ["IN_REPAIR"]: "#d69e2e",
                STATUS_UZ["LOST"]: "#e53e3e",
    ...
    if stats['by_dept']:
        ...
        fig3 = px.bar(
            x=list(stats['by_dept'].values()),
            y=list(stats['by_dept'].keys()),
            orientation='h',
            color_discrete_sequence=["#1a3a6e"]
        )
    ...
    if history:
        ...
        for h in history:
            rows.append({
                ...
                "Eski status": STATUS_UZ.get(h.get("old_status",""), h.get("old_status","") or "—"),
                "Yangi status": STATUS_UZ.get(h.get("new_status",""), h.get("new_status","") or "—"),
                "Kim": h["changed_by"],
                "Sana": h["created_at"][:16]
            })
...
# ═══════════════════════════════════════════════════════════
    ...
    tabs = st.tabs(["📋 Ro'yxat", "➕ Qo'shish", "🔍 Aktiv ko'rish / QR"])
            ...
            for asset in assets:
                ...
                with st.expander(f"{emoji} **{asset['name']}** — {asset['serial_number']} | {STATUS_UZ[asset['status']]}"):
                    ...
                    with c2:
                        ...
                        st.write(f"**Qo'shilgan:** {asset['created_at'][:10]}")
                    with c3:
                        # Status change
                        ...
                        if allowed_next:
                            new_st = st.selectbox(
                                "Yangi status",
                                allowed_next,
                                format_func=lambda x: STATUS_UZ[x],
                                key=f"st_{asset['id']}"
                            )
            ...
            if asset:
                ...
                with col1:
                    ...
                    if st.button("🤖 AI Risk tahlili", key="risk_btn"):
                        ...
                        getattr(st, level_color)(f"{risk_emoji} **Risk: {risk['risk_level']}** — {risk['recommendation']} *(manba: {risk['source']})*")

                with col2:
                    st.subheader("📱 QR Kod")
                    # Current owner
                    active_assignments = [a for a in get_assignments(active_only=True) if a["asset_id"] == asset_id]
                    owner = active_assignments[0]["employee_name"] if active_assignments and active_assignments[0].get("employee_name") else "Tayinlanmagan"
                    qr_b64 = generate_qr(asset, owner)
                    st.image(
                        base64.b64decode(qr_b64),
                        caption=f"QR: {asset['serial_number']}",
                        width=220
                    )
                    st.download_button(
                        "⬇ QR yuklab olish",
                        data=base64.b64decode(qr_b64),
                        file_name=f"qr_{asset['serial_number']}.png",
                        mime="image/png",
    ...
    with tabs[0]:
        ...
        if not employees:
            ...
            df = pd.DataFrame([{
                "ID": f"#{e['id']}",
                "F.I.O": e["full_name"],
                "Xodim ID": e["employee_id"],
                "Bo'lim": e["department"],
                "Filial": e.get("branch") or "—",
                "Email": e.get("email") or "—",
                "Telefon": e.get("phone") or "—",
                "Qo'shilgan": e["created_at"][:10]
            } for e in employees])
            ...
            if st.button("O'chirish", type="secondary"):
                delete_employee(del_opts[del_sel])
                st.success("O'chirildi!")
                st.rerun()
...
# ═══════════════════════════════════════════════════════════
            ...
            for asgn in assignments:
                with st.expander(f"🔗 **{asgn['asset_name']}** → {asgn.get('employee_name') or 'Bo\'lim: '+asgn.get('department','—')}"):
                    ...
                    with c1:
                        ...
                        st.write(f"**Kategoriya:** {asgn['category']}")
                    with c2:
                        st.write(f"**Xodim:** {asgn.get('employee_name') or '—'}")
                        st.write(f"**Bo'lim:** {asgn.get('department') or asgn.get('emp_dept') or '—'}")
                        st.write(f"**Filial:** {asgn.get('branch') or '—'}")
                        st.write(f"**Tayinlangan:** {asgn['assigned_at'][:16]}")
                    with c3:
    ...
    with tabs[1]:
        ...
        if all_asgn:
            df = pd.DataFrame([{
                "Aktiv": a["asset_name"],
                "Xodim": a.get("employee_name") or "—",
                "Bo'lim": a.get("department") or "—",
                "Tayinlangan": a["assigned_at"][:16],
                "Qaytarilgan": a.get("returned_at","")[:16] if a.get("returned_at") else "—",
    ...
    with tabs[2]:
        ...
        if not available:
            st.warning("⚠️ Tayinlash uchun mavjud aktivlar yo'q (REGISTERED statusidagi aktivlar kerak)")
        elif not employees:
            ...
            with st.form("assign_form"):
                ...
                if st.form_submit_button("🔗 Tayinlash", type="primary", use_container_width=True):
                    ok, msg = assign_asset(
                        asset_id=asset_opts[sel_asset],
                        employee_id=emp_opts[sel_emp],
                        department=dept,
                        branch=branch,
                        notes=notes
                    )
...
# ═══════════════════════════════════════════════════════════
    ...
    if not history:
        ...
        st.caption(f"Jami {len(history)} ta yozuv ko'rsatilmoqda")
